...
Google ZHONGHUA secondary in wikipedia and insert manually into csv: 1.3887°N 103.7652°E
...
schools_sf <- st_as_sf(schools, 
                       ...
                       crs = 4326) %>% #identify spatial type
  st_transform (crs = 3414) #convert spatial type
...
```{r}
tmap_options(check.and.fix = TRUE) #boundary map, and TRUE to force close the polygon
tm_shape(mpsz) + 
  tm_polygons() + #always plot the polygon first, then on points
tm_shape(schools_sf) +
  tm_dots()
```
...
It is always a good practice to examine the summary statistics of the derived variable. Using the steps you learned in previous exercises, compute and display the summary statistics of sch_count field

```{r}
summary(mpsz$SCHOOL_COUNT)
```

Important The summary statistics above reveals that there are excessive 0 values in SCHOOL_COUNT field. If log() is going to use to transform this field, additional step is required to ensure that all 0 will be replaced with a value between 0 and 1 but not 0 neither 1.4
