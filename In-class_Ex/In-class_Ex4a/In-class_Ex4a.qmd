---
title: "In-class Exercise 4a: Calibrating Spatial Interaction Models with R"
...
Do it yourself Using the steps you learned in previous exercises, load the above packages onto R environment.
...
Important Notice that this data.frame include intra-zonal flow.
...
Things to learning from the conde chunk For origin-constrained model, only explanatory variables representing the attractiveness at the destinations will be used. All the explanatory variables including distance will be log transformed. ORIGIN_SZ is used to model 𝜇𝑖 . It must be in categorical data type. It is important to note that -1 is added in the equation after the distance variable. The -1 serves the purpose of removing the intercept that by default, glm will insert into the model.

What can we learn from the report above? the ⍺1 and ⍺2 of SCHOOL_COUNT and BUSINESS_COUNT are 0.4755516 and 0.1796905 respectively.
...
Do it yourself What statistical conclusion you can draw from the report above?
...
Do it yourself Using the R-Squared function written earlier, compute the R-Squared of the Doubly-constrined Model.
...
Tip Notice that rename() is used to rename the field name and the \$ in the original field name has been replaced with an .. This is because R replaced \$ with . during the cbind().

Do it yourself Repeat the same step for Doubly Constrained Model (i.e. dbcSIM_Poisson)
...
Quiz What observation can you draw from the figure above and what follow-up action will you take?

Additional notes for awareness:

-   *To observe for the 3 variables at the bottom of the list :log(SCHOOL_COUNT) + log (RETAIL_COUNT) + log(DIST) = alpha, beta and gamma value.*

-   *For distance is must be negative here, as further the DIST less willingness to travel. It has inverse relationship to attraction. If model indicates +, must relook.*

-   *p value at the most right must be small (i.e. bolew -.05.*

-   *glm don't calculate R sq, hence need to calculate.*

-   *double constraint / dbscSIM = no need -1*
