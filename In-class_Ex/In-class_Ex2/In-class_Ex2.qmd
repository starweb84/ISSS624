...
The below code chunk to display nb_queen sf tibble data frame in a table display.
...
Derive a contiguity neighbour list using lag 2 Queen's method.
...
Things to learn from the code chunk above st_contiguity() of sfdep is used to identify the neighbours by using contiguity criteria. The output is a list of neighbours (i.e. nb). st_inverse_distance() is then used to calculate inverse distance weights of neighbours on the nb list.
